...
import { WHATSAPP_NUMBER, INSTAGRAM_URL, INSTAGRAM_HANDLE, GYM_LOCATION, GOOGLE_MAPS_URL } from "@/turnos/config";
...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Yo muestro un toast porque este formulario es visual (sin backend por ahora).
    toast({
      title: "Mensaje listo",
      description: "Escribinos por WhatsApp o Instagram y lo vemos.",
    });
  ...
  const waMessage = encodeURIComponent(
    `Hola! Soy ${form.nombre || "____"}. Mi contacto es ${form.contacto || "____"}.\n\n${form.mensaje || "Quiero consultar por planes y turnos."}`
  );
          ...
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 animate-on-scroll">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button type="submit" className="btn-outline-glow flex items-center justify-center gap-2">
                  <Send size={16} />
                  Preparar mensaje
                </button>

                {/* Yo dejo el botón directo a WhatsApp con el mensaje ya armado */}
                <a
                  href={waLink}
                  target="_blank"
            ...
            <div className="space-y-6 animate-on-scroll">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="glass-card p-5 flex items-center gap-4 glow-hover"
              >
                <MessageCircle className="text-primary" size={24} />
                <div>
                  <p className="font-semibold text-sm">WhatsApp</p>
                  <p className="text-muted-foreground text-xs">
                    +54 9 {WHATSAPP_NUMBER.replace(/^549/, "")}
                  </p>
                </div>
              </a>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">@{INSTAGRAM_HANDLE}</p>
                </div>
              </a>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">{GYM_LOCATION}</p>
                </div>
              </a>
            </div>
          </div>
