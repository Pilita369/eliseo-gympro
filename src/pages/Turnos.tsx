...
import { PLANS, DAYS_CONFIG, TIME_SLOTS, MAX_PER_SLOT, DEFAULT_PRICE_BY_PLAN } from "@/turnos/config";
import { loadSlots, saveSlots, loadReservations, saveReservations } from "@/turnos/storage";
import { generateReservationId } from "@/turnos/reservation";
...
export default function Turnos() {
  ...
  const currentSlotKey = currentDay && selectedTime ? `${currentDay.dateISO}_${selectedTime}` : "";
  ...
  const handleBlock = () => {
    ...
    persistSlots(nextSlots);
    toast({ title: "Turno bloqueado", description: `${currentDay.label} ${selectedTime} — 2 cupos` });
    setBlockConfirmOpen(false);
    setSelectedTime(null);
  };

  // Marcar comprobante enviado
  const handleMarkSent = (id: string) => {
    const next = reservations.map((r) =>
      r.id === id ? { ...r, status: "Comprobante enviado" as const } : r
    );
    persistReservations(next);
    setReceiptReservation((prev) => prev && prev.id === id ? { ...prev, status: "Comprobante enviado" } : prev);
      ...
      <Dialog open={blockConfirmOpen} onOpenChange={setBlockConfirmOpen}>
        <DialogContent className="glass-card border-[hsl(var(--glass-border))] bg-[hsl(220_18%_10%)]">
          ...
          <DialogFooter className="gap-2">
            <Button variant="ghost" onClick={() => setBlockConfirmOpen(false)}>Cancelar</Button>
            <Button variant="destructive" onClick={handleBlock}>Bloquear</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!cancelTarget} onOpenChange={(v) => !v && setCancelTarget(null)}>
        <DialogContent className="glass-card border-[hsl(var(--glass-border))] bg-[hsl(220_18%_10%)]">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button variant="destructive" onClick={confirmCancel}>Cancelar reserva</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
