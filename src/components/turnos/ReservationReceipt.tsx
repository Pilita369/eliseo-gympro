// Modal/tarjeta de confirmación tras reservar — incluye datos de pago, QR, WhatsApp, ICS
...
import {
  TRANSFER_ALIAS, TRANSFER_CBU, TRANSFER_OWNER,
  CURRENCY, QR_IMAGE_PATH, GYM_NAME,
} from "@/turnos/config";
        ...
        <div className="space-y-5">
          ...
          <div className="glass-card p-4 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-muted-foreground">Nº Reserva</span>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm">{r.plan}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-muted-foreground">Día</span>
              <span className="text-sm">{r.dayLabel} ({r.dateISO})</span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm">1 lugar reservado (máx 2 por turno)</span>
            </div>
            {r.amountSuggested && (
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">Monto sugerido</span>
                <span className="text-sm font-semibold text-primary">{CURRENCY} {r.amountSuggested}</span>
              </div>
            )}
          </div>
          ...
          <p className="text-xs text-muted-foreground text-center leading-relaxed">
            Para confirmar tu turno, realizá la transferencia y enviá el comprobante por WhatsApp o Email.
          </p>
          ...
          <div className="flex flex-col items-center gap-2">
            {!qrError ? (
              ...
              <div className="w-40 h-40 rounded-lg border border-dashed border-muted-foreground/30 flex flex-col items-center justify-center gap-1 text-muted-foreground">
                <Image size={24} />
                <span className="text-[10px] text-center leading-tight px-2">Subí tu QR a<br/>public/qr-transferencia.png</span>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button variant="outline" className="gap-1.5 text-xs" onClick={() => window.open(buildWhatsAppUrl(r), "_blank")}>
              <MessageCircle size={14} /> WhatsApp
            </Button>
            <Button variant="outline" className="gap-1.5 text-xs" onClick={() => window.open(buildMailtoUrl(r), "_blank")}>
              <Mail size={14} /> Email
            </Button>
            <Button variant="outline" className="gap-1.5 text-xs" onClick={() => copyText(buildMessage(r), "Mensaje")}>
              <Copy size={14} /> Copiar mensaje
            </Button>
            <Button variant="outline" className="gap-1.5 text-xs" onClick={() => downloadICS(r)}>
          ...
          {r.status === "Pendiente de pago" && (
            <Button className="w-full btn-cta" onClick={() => onMarkSent(r.id)}>
